...
def _make_image_folder(tmp_path: Path, subdirs: list[str], counts: list[int]) -> str:
    """A photo folder: one subfolder per name, holding that many small RGB TIFFs. 40x60 is
    big enough for napari to treat the last axis as RGB (it won't below ~30 px a side); the
    loader's crop to the real view width quietly does nothing on images this small."""
    for subdir, n in zip(subdirs, counts):
        (tmp_path / subdir).mkdir()
        for i in range(n):
            tf.imwrite(tmp_path / subdir / f"img{i}.tif", np.zeros((40, 60, 3), "uint8"))
    return str(tmp_path)


def test_load_data(cpt_widget: ParticleTracksWidget, tmp_path: Path):
    """A valid folder (three view subfolders, same number of images, more than one each)
    becomes a 4D view/event/y/x image layer, and a process added afterwards records the event
    on screen. Calls _load_data_from directly rather than going through the folder dialog, which
    would also save the pytest temp folder into the developer's real "last images folder"
    setting."""
    folder = _make_image_folder(tmp_path, ["my_view1", "my_view2", "my_view3"], [3, 3, 3])
    cpt_widget._load_data_from(folder)

    assert cpt_widget.viewer.layers[IMAGE_LAYER_NAME].ndim == 4
    assert cpt_widget.viewer.dims.current_step[1] == 0

    cpt_widget.viewer.dims.set_current_step(1, 1)
    cpt_widget.particle_decays_menu.setCurrentIndex(1)
    assert cpt_widget.data[0].event_number == 1
    assert (
        cpt_widget.table.item(0, cpt_widget._get_table_column_index("event_number")).text()
        == "1"
    )
...
@pytest.mark.parametrize(
    "subdirs, counts",
    [
        (["my_view1", "my_view2"], [2, 2]),  # only two views
        (["my_view1", "my_view2", "my_view3"], [1, 2, 2]),  # unequal image counts
        (["my_view1", "my_view2", "no_view"], [2, 2, 2]),  # a folder not named as a view
        ([], []),  # no view folders at all
    ],
)
def test_load_data_rejects_invalid_folder(cpt_widget, tmp_path, subdirs, counts):
    """An invalid folder loads nothing and shows the warning (its wording isn't checked)."""
    cpt_widget._load_data_from(_make_image_folder(tmp_path, subdirs, counts))
    assert IMAGE_LAYER_NAME not in cpt_widget.viewer.layers
    assert cpt_widget.msg.isVisible()


def test_load_data_with_cancelled_dialog(cpt_widget):
    """A cancelled folder dialog hands the loader an empty name - loads nothing, shows the warning."""
    cpt_widget._load_data_from("")
    assert IMAGE_LAYER_NAME not in cpt_widget.viewer.layers
    assert cpt_widget.msg.isVisible()
